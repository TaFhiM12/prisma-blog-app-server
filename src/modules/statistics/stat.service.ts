...
const getStat = async () => {
    return await prisma.$transaction(async (tx) => {
        const [totalPosts, publishedPostss, draftPosts, archivePosts, totalComment, totalApprovedComment, totalRejectComment, userCount, adminRoleCount, userRoleCount, viewsCount] =
            await Promise.all([
                await tx.post.count(),
